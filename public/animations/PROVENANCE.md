...
## Standing project authorization and upstream rights

- Manifest authorization family: `thornvale-animation-project`
- Runtime source families: `friendsies-animations` and
  `friendsies-story-actions-v1`
- Manifest status: `project-release-authorized`
- Release blocked: no
- Project authorization: on 2026-07-14, the Thornvale project owner granted
  standing publication authorization for all present and future animation
  assets and derivatives the owner controls or may lawfully use as integrated
  parts of Thornvale. Authorization follows the asset kind and integrated
  project use rather than individual items.
...
- Upstream-rights gate: every runtime animation family must separately record a
  verified source or project-authored status, evidence, source identity,
  hashes, and transforms. The standing project grant cannot admit unknown or
  prohibited upstream material.
...
The source repository has no general `LICENSE` file. Its recorded ownership and
the Adobe game-use terms establish the upstream chain for this family; the
standing Thornvale animation authorization supplies the project publication
scope. Source files and standalone animation assets remain outside that scope.

`PIZZALORD713/animation_collection` was audited but is not copied here. Several
of its source files identify Mixamo provenance. A future intake inherits the
standing Thornvale animation authorization, but must still record and pass its
own upstream source, license, hash, transform, fallback, budget, and QA gates.
...
- `node --test tests/asset-budgets.test.js` verifies the standing authorization,
  source-family inheritance, upstream-rights status, fallback, QA evidence,
  hashes, transforms, and budgets.
- `npm run check`: 153/153 tests passed on the `origin/main` integration, the
  ...
  budget.
- `npm run assets:release`: `friendsies-animations` and the standing-authorized

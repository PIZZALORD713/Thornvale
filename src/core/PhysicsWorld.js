...
import {
  BoxGeometry, MeshStandardMaterial, Mesh, SphereGeometry,
  BufferGeometry, Float32BufferAttribute, LineBasicMaterial, LineSegments,
} from 'three';
...
export class PhysicsWorld {
  ...
  createGround(size = 50, scene) {
    ...
    // Three.js visual
    if (scene) {
      const groundGeo = new BoxGeometry(size * 2, 0.2, size * 2);
  ...
  updateDebugRender(scene) {
    // Early-return when debug is off — avoids the WASM debugRender() call

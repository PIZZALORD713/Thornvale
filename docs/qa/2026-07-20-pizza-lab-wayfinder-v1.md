...
Fresh headed Chromium smoke on the integrated production preview:
...
- Runtime inspection found `authored_wayfinder` at `(0, 0, -6.4)` with asset
  ...
  children attached.
- The production-minified `draco_wasm_wrapper.js` decoded the standalone
  Wayfinder and bundled character geometry successfully. The newer mainline
  objective-cue DOM and `objectiveHintTrail` owner remained present in both
  asset modes.
...
The isolated Pizza Lab branch passed the original production gate at 8,388,275
of 8,388,608 bytes. Integrating the newer object-cue mainline produced an
8,416,553-byte pre-optimization build, 27,945 bytes above that old cap. ADR 0009
raises only the complete-dist ceiling by 20% while preserving every
family-specific cap. Decode-verified production minification of the copied
Draco WASM wrapper reclaimed 489 bytes, leaving the combined mainline artifact
at 8,416,064 of 10,066,330 bytes with 1,650,266 bytes of headroom.

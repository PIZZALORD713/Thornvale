...
| `pizza_terrain_contract` | None; terrain is preview-only in v0.1 |

Creation, deletion, arbitrary Python, terrain mutation, and publishing are
intentionally unavailable in this pilot.
...
when inspecting a file, `PIZZA_LAB_BLEND_FILE`. Headless v0.1 supports inspect,
...
The Blender verification creates a disposable factory scene, then proves inspect,
dry-run, apply, undo, validation, and the terrain boundary without writing an
asset. It compares canonical transforms, not `.blend` bytes.

## Next bounded gate

Install the add-on in a separate Blender profile and repeat the fixture transform
interactively. Compare its canonical scene snapshot with the headless result.
Only after that passes should Pizza Lab add object creation or a versioned export
workflow. Terrain authoring remains a separate later contract because ThornVale's
render and Rapier geometry currently share JavaScript authority.
